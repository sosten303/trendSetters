...
if(isset($_POST["Submit"])){
 ...
 // $DateTime = strftime("%Y-%m-%d %H:%M:%S",$CurrentTime);
  ...
  if (empty($Category) || empty($Service) || empty($ServiceTitle)){
  $_SESSION["ErrorMessage"]="Category, Service and Image are Required !";
  Redirect_to("adminservice.php");

}else{
  $Query="INSERT INTO service(date_published, service_text, service_bunner, category, service_title, expert_id )
           VALUES('$date_published', '$Service', '$ServiceBunner', '$Category', '$ServiceTitle', '$ExpertId')";

  $Excute=mysqli_query($dbcon,$Query);
  move_uploaded_file($_FILES["service_bunner"]["tmp_name"],$Target);
  if ($Excute) {
     $_SESSION["SuccessMessage"]="Service Shared successfully";
     Redirect_to("IndividualService.php");
  }else{
...
<!-- Expert navbar -->
 <div class="row">
   <div class="col-md-4">
     ...
     <a href="adminservice.php" id="dashboard-active"><i class="fas fa-box"></i>Share Service</a>
     ...
     <div class="col-md-8">
      <div class="addNewPost-container">
       ...
       <!-- ********************************************** -->
        <!-- portfolio form -->
        <div>
          <form action="adminservice.php" method="post" enctype="multipart/form-data"/>

            <label for="ServiceImage">Service Image</label>
            <p></p>
            <i class="fas fa-camera"></i> <input type="File" name="service_bunner" id="ServiceImage"/>
            <p></p>
            ...
            <p></p>
            <input type="text" name="service_title" id="Title" placeholder=" This is a heading" />
            <p></p>
            <label for="Service">Service</label>
            <p></p>
            <textarea name="service_text" id="Service" placeholder="Share your Service here"></textarea>
            <p></p>
            <input type="submit" name="Submit" value="Share Service"/>
          </form>
      </div>
       </div>
     </div>
