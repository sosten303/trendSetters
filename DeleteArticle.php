...
if(isset($_POST["Submit"])){
 ...
 // $DateTime = strftime("%Y-%m-%d %H:%M:%S",$CurrentTime);
  ...
  if (isset($_GET["DeleteArticle"])) {
     $DeleteArticleFromUrl = $_GET["DeleteArticle"];
  // global $dbcon;
  // $ExpertId = $_SESSION["User_Id"];
  $Query= "DELETE FROM article WHERE article.id = '$DeleteArticleFromUrl'";
 ...
 if ($Excute) {
    ...
    $_SESSION["ErrorMessage"]=" Failed to Delete Article Try Again !";
    Redirect_to("DeleteArticle.php");
  }
}
     ...
     <a href="updateProfile.php"><i class="fas fa-user"></i>Update Profile</a>
      <?php
        ...
        if ($TotalRaws) {
          ...
          <a href="IndividualArticles.php"  id="dashboard-active"><i class="fas fa-newspaper"></i> Articles <span style="background-color: #00b4ff; color: #ffffff; padding: 1px 3px; border-radius: 2px;"><?php echo $TotalRaws; ?></span></a>
     ...
     <div class="col-md-8">
      <div class="addNewPost-container">
       ...
       <?php 
          ...
          $DeleteArticleFromUrl = $_GET["DeleteArticle"];
          $Query = "SELECT * FROM article INNER JOIN expert_bio ON article.expert_id = expert_bio.id WHERE article.id = '$DeleteArticleFromUrl'";
          $Excute = mysqli_query($dbcon,$Query);
        ...
        <form action="DeleteArticle.php?DeleteArticle=<?php echo $DeleteArticleFromUrl; ?>" method="post" enctype="multipart/form-data">
           <p></p>
         <span>Your Profile Picture:</span> 
         <img disabled src="Upload/<?php echo $ArticleBunner; ?>" with=270px height=100px;>
         <p></p>
         <label>Change Profile Picture :</label>
         <i class="fas fa-camera"></i> <input disabled type="File" name="article_bunner">
         <p></p>
         <label for="Title">Title :</label>
         <input disabled type="text" name="article_title" value="<?php echo $ArticleTitle ?>">
         <p></p>
          <label>Article :</label>
         <p></p>
         <textarea disabled name="article_text" placeholder="Edit article"><?php echo $Article; ?></textarea>
         <p></p>
