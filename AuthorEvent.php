...
<html>
<head>
  <title>Experts Events</title>
  <meta charset="utf-8">
  <meta lang="eng">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="stylesheet" type="text/css" href="css/bootstrap-grid.min.css">
  <link rel="stylesheet" type="text/css" href="fontawesome/css/all.css">
  <link rel="stylesheet" type="text/css" href="css/AuthorEvent.css">
</head>
<body>
   <!-- navigation bar -->
    <!-- heading -->
...
<!-- Expert navbar -->
 <div class="row">
   <div class="col-md-3">
     ...
     <?php } ?> <!-- End if Condition -->
     <a href="AuthorPortfolio.php?Page=1"><i class="fas fa-user-tie"></i>Experts Portfolios</a>
     ...
     <?php
        ...
        if ($TotalRaws) {
          ...
          <a href="IndividualEvent.php?Page=1"><i class="fas fa-calendar-alt"></i> Event <span style="background-color: #00b4ff; color: #ffffff; padding: 1px 3px; border-radius: 2px;"><?php echo $TotalRaws; ?></span></a>
     <?php } ?> <!-- End if Condition -->

     <a href="AuthorEvent.php?Page=1" id="dashboard-active"><i class="fas fa-calendar-alt"></i>Experts Events</a>
      ...
      <div class="addNewPost-container">
       ...
       //Query when Search Button is Active
           if (isset($_GET["search-btn"])) {
               $Search = $_GET["search"];
               $ExpertId = $_SESSION["User_Id"];
               $ViewQuery = "SELECT event.id, fname, lname, profile_pic, email, title, profession, date_published, event_descrip, event_bunner, theme, venue, event_time, event_date, category, expert_id FROM client INNER JOIN expert_bio ON event.expert_id = expert_bio.id WHERE date_published LIKE '%$Search%' OR event_time LIKE '%$Search%'  OR event_date LIKE '%$Search%' OR theme LIKE '%$Search%' OR venue LIKE '%$Search%' OR category LIKE '%$Search%'";
          ...
          $ViewQuery = "SELECT event.id, fname, lname, profile_pic, email, title, profession, date_published, category, theme, venue, event_descrip, event_date, event_bunner, event_time FROM event INNER JOIN expert_bio ON event.expert_id = expert_bio.id  ORDER BY date_published DESC LIMIT $ShowPostFrom,5";}
           else{
            $ExpertId = $_SESSION["User_Id"];
            $ViewQuery = "SELECT event.id, fname, lname, profile_pic, email, title, profession, date_published, category, theme, venue, event_descrip, event_date, event_bunner, event_time FROM event INNER JOIN expert_bio ON event.expert_id = expert_bio.id ORDER BY date_published DESC LIMIT 0,5";}
        ...
        <div class="event_content">
         <h1><?php echo $Theme; ?></h1>
          ...
          <div class="event_details">
           ...
           <p><b>Date Posted :</b> <?php echo $DatePublished; ?></p>
           <div class="event-theme">
            ...
            <p>
             ...
             ?>
             <a href="AuthorFullEvent.php?id=<?php echo $EventId ?>">Read More</a>
           </p>
         </div>
         </div>
         <div class="expert_event">
          <a href="authorProfile.php?id=<?php echo $ExpertId ?>">
            <img src="ProfilePic/<?php echo $ProfilePic; ?>" id="profile-pic" vspace=12 hspace=10 />
...
<div class="article-pagination">
      ...
      ?>
      <a href="AuthorEvent.php?Page=<?php echo $Page-1?>"> &laquo; </a> 
      <?php } ?>
      <?php } ?>
    <!-- end Back btn -->
     ...
     <!-- Adding links to pagination -->
        <?php
         $QueryPagination = "SELECT COUNT(*) FROM event INNER JOIN expert_bio ON event.expert_id = expert_bio.id";
    ...
    <a href="AuthorEvent.php?Page=<?php echo $i; ?>" id="article-pagination-active"><?php echo $i; ?></a>
    <?php
      }else{
    ?>
    <a href="AuthorEvent.php?Page=<?php echo $i; ?>"><?php echo $i; ?></a>
  <?php } ?> <!-- end else -->
<?php } ?>
          <?php } ?> <!-- end forloop -->
    <!-- Forward btn -->
      <?php
        ...
        ?>
           <a href="AuthorEvent.php?Page=<?php echo $Page+1 ?>"> &raquo; </a>
          <?php } ?>
          <?php } ?> <!-- End Forward btn -->
    </div>
      <!-- End Pagination -->
