...
if(isset($_POST["Submit"])){
 ...
 // $DateTime = strftime("%Y-%m-%d %H:%M:%S",$CurrentTime);
  ...
  if (empty($Bio)){
  $_SESSION["ErrorMessage"]="Your Biography is required";
  Redirect_to("create-profile.php");

}else{
  // $_SESSION["SuccessMessage"]="Your Biography Created successfully";
  // Redirect_to("create-profile.php");
  // global $dbcon;
  // $BioIdFromURL=$_GET['id'];
  $Query="INSERT INTO expert_bio(datetime, fname, lname, profile_pic, bio)
           VALUES('$DateTime', '$Admin', '$Image', '$Bio')";

  $Excute=mysqli_query($dbcon,$Query);
  move_uploaded_file($_FILES["profile_pic"]["tmp_name"],$Target);
  if ($Excute) {
     $_SESSION["SuccessMessage"]="Biography Created successfully";
     Redirect_to("expertProfiles.php");
  }else{
 ...
 <div class="row">
   <div class="col-md-4">
    <div class="dashboard">
     ...
     <a href="addnew-post.php"><i class="fas fa-plus"></i>Add New Post</a>
     <a href="categories.php"><i class="fas fa-tags"></i>Categories</a>
     <a href="expertadmins.php"><i class="fas fa-user-cog"></i>Manage Admins</a>
     <a href="create-profile.php" id="dashboard-active"><i class="fas fa-user-cog"></i>Create Profile</a>
     ...
     <div class="col-md-8">
      <div class="addNewPost-container">
       ...
       ?>
         ...
         $Query = "SELECT * FROM expert_bio ORDER BY datetime desc";
         $Excute = mysqli_query($dbcon,$Query);
         while($DataRows = mysqli_fetch_array($Excute)) {
           $BioId = $DataRows['id'];
           $ExpertImage = $DataRows['profile_pic'];
           $ExpertName = $DataRows['expertname'];
           $ExpertBio = $DataRows['bio'];
           // $BioToAdminPanelId = $DataRows["admin_panel_id"];
           if (isset($_SESSION["UserName"]) == $ExpertName) {
             echo "you can Edit this profile";
           }
         ?>
         <div class="UserComment">
          <img src="ProfilePic/<?php echo $ExpertImage; ?>" with=10; height=70;>
          <a href="expertEditProfile.php?EditProfile=<?php echo $BioId;?>" id="edit-btn">Edit Profile</a>
         </div>
       <?php } ?>
...
<!-- ************************ form ************************* -->
       <form action="create-profile.php" method="post" enctype="multipart/form-data">
         ...
         <p></p>
         <input type="submit" name="Submit" value="Create Biography">
       </form>
       </div>
     </div>
     </div>
