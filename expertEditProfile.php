...
if(isset($_POST["Submit"])){
 ...
 // $DateTime = strftime("%Y-%m-%d %H:%M:%S",$CurrentTime);
  ...
  $EditFromURL=$_GET['EditProfile'];
  $Query= "UPDATE expert_bio SET datetime = '$DateTime', fname='$Fname', lname = '$Lname', profile_pic='$Image', bio='$Bio'
  WHERE id='$EditFromURL'";

  $Excute=mysqli_query($dbcon,$Query);
  move_uploaded_file($_FILES["profile_pic"]["tmp_name"],$Target);
  if ($Excute) {
     $_SESSION["SuccessMessage"]="Biography Updated successfully";
    ...
    <div class="dashboard">
     ...
     <div class="col-md-8">
       ...
       <div>
          <?php
          // global $dbcon;
          $SearchQueryParameter=$_GET['EditProfile'];
          $Query="SELECT * FROM expert_bio WHERE id = '$SearchQueryParameter'";
          ...
          while ($DataRows = mysqli_fetch_array($ExcuteQuery)) {
             ...
             $ImageToBeUpdated=$DataRows['profile_pic'];
             $BioToBeUpdated=$DataRows['bio'];
          }
       ?>
       <!-- ********************** Form Start ****************************** -->
       <form action="expertEditProfile.php?EditProfile=<?php echo $SearchQueryParameter; ?>" method="post" enctype="multipart/form-data">
        <p><?php echo $Fname; ?></p>
          <p></p>
         <span>Existing Image:</span> 
         <img src="ProfilePic/<?php echo $ImageToBeUpdated; ?>" with=270px height=100px;>
         <p></p>
