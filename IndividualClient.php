...
<html>
<head>
	<title>Individual Client</title>
	<meta charset="utf-8">
	<meta lang="eng">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<link rel="stylesheet" type="text/css" href="css/bootstrap-grid.min.css">
	<link rel="stylesheet" type="text/css" href="fontawesome/css/all.css">
	<link rel="stylesheet" type="text/css" href="css/IndividualClient.css">
</head>
<body>
   <!-- navigation bar -->
    <!-- heading -->
     ...
     <a href="adminclient.php"><i class="fas fa-user-plus"></i>Add Clients</a>
      <?php
        ...
        if ($TotalRaws) {
          ...
          <a href="IndividualClient.php?Page=1" id="dashboard-active"><i class="fas fa-user-tag"></i> Client <span style="background-color: #00b4ff; color: #ffffff; padding: 1px 3px; border-radius: 2px;"><?php echo $TotalRaws; ?></span></a>
     <?php } ?> <!-- End if Condition -->
     ...
     <div class="col-md-7">
      <div class="addNewPost-container">
       ...
       <div class="searchForm">
            <form action="IndividualClient.php">
       ...
       //Query when Search Button is Active
           if (isset($_GET["search-btn"])) {
               $Search = $_GET["search"];
               $ExpertId = $_SESSION["User_Id"];
               $ViewQuery = "SELECT client.id, fname, lname, profile_pic, email, title, profession, date_published, client_name, client_bunner, about_client, expert_id FROM client INNER JOIN expert_bio ON client.expert_id = expert_bio.id AND expert_id = '$ExpertId' WHERE date_published LIKE '%$Search%' OR client_name LIKE '%$Search%' OR about_client LIKE '%$Search%'";
          ...
          $ExpertId = $_SESSION["User_Id"];
           $ViewQuery = "SELECT client.id, fname, lname, profile_pic, email, title, profession, date_published, client_name, client_bunner, about_client FROM client INNER JOIN expert_bio ON client.expert_id = expert_bio.id WHERE expert_bio.id = '$ExpertId' ORDER BY date_published DESC LIMIT $ShowPostFrom,5";}
           else{
            $ExpertId = $_SESSION["User_Id"];
           $ViewQuery = "SELECT client.id, fname, lname, profile_pic, email, title, profession, date_published, client_name, client_bunner, about_client FROM client INNER JOIN expert_bio ON client.expert_id = expert_bio.id WHERE expert_bio.id = '$ExpertId' ORDER BY date_published DESC LIMIT 0,5";}
           ...
           while ($DataRows = mysqli_fetch_array($Execute)) {
             ...
             $ClientName = $DataRows["client_name"];
             $ClientBunner = $DataRows["client_bunner"];
             $AboutClient = $DataRows["about_client"];
           ...
           <div class="client_content">
            ...
            <p id="client_date"> <i class="fas fa-calendar-day"></i> <?php echo $DatePublished; ?></p>
            <img src="ClientPic/<?php echo $ClientBunner; ?>" />
            <p>
              <?php
              if (strlen($AboutClient)>150) {
                $AboutClient = substr($AboutClient, 0,150).'..';
              }
               echo nl2br($AboutClient);
             ?>
             <a href="IndividualFullClient.php?id=<?php echo $ClientId ?>">Read More</a>
           </p>
...
<div class="article-pagination">
      ...
      ?>
      <a href="IndividualClient.php?Page=<?php echo $Page-1?>"> &laquo; </a> 
      <?php } ?>
      <?php } ?>
    <!-- end Back btn -->
     ...
     <!-- Adding links to pagination -->
        <?php
         $QueryPagination = "SELECT COUNT(*) FROM client INNER JOIN expert_bio ON client.expert_id = expert_bio.id";
    ...
    <a href="IndividualClient.php?Page=<?php echo $i; ?>" id="article-pagination-active"><?php echo $i; ?></a>
    <?php
      }else{
    ?>
    <a href="IndividualClient.php?Page=<?php echo $i; ?>"><?php echo $i; ?></a>
     <?php } ?> <!-- end else -->
     <?php } ?>
          <?php } ?> <!-- end forloop -->
    <!-- Forward btn -->
      <?php
        ...
        ?>
           <a href="IndividualClient.php?Page=<?php echo $Page+1 ?>"> &raquo; </a>
          <?php } ?>
          <?php } ?> <!-- End Forward btn -->
          </div>
      <!-- End Pagination -->
