...
<html>
<head>
	<title>Individual Events</title>
	<meta charset="utf-8">
	<meta lang="eng">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<link rel="stylesheet" type="text/css" href="css/bootstrap-grid.min.css">
	<link rel="stylesheet" type="text/css" href="fontawesome/css/all.css">
	<link rel="stylesheet" type="text/css" href="css/IndividualEvent.css">
</head>
<body>
   <!-- navigation bar -->
    <!-- heading -->
...
<!-- Expert navbar -->
     ...
     <a href="adminevent.php"><i class="fas fa-calendar-alt"></i>Share Event</a>
      <?php
        ...
        if ($TotalRaws) {
          ...
          <a href="IndividualEvent.php?Page=1" id="dashboard-active"><i class="fas fa-calendar-alt"></i> Event <span style="background-color: #00b4ff; color: #ffffff; padding: 1px 3px; border-radius: 2px;"><?php echo $TotalRaws; ?></span></a>
     <?php } ?> <!-- End if Condition -->

     <a href="AuthorEvent.php?Page=1"><i class="fas fa-calendar-alt"></i>Expert Events</a>
     ...
     <div class="col-md-7">
      <div class="addNewPost-container">
       ...
       <div class="searchForm">
            <form action="IndividualEvent.php">
       ...
       //Query when Search Button is Active
           if (isset($_GET["search-btn"])) {
               $Search = $_GET["search"];
               $ExpertId = $_SESSION["User_Id"];
               $ViewQuery = "SELECT event.id, fname, lname, profile_pic, email, title, profession, date_published, event_descrip, event_bunner, theme, venue, event_time, event_date, category, expert_id FROM event INNER JOIN expert_bio ON event.expert_id = expert_bio.id AND expert_id = '$ExpertId' WHERE date_published LIKE '%$Search%' OR event_time LIKE '%$Search%'  OR event_date LIKE '%$Search%' OR theme LIKE '%$Search%' OR venue LIKE '%$Search%' OR category LIKE '%$Search%'";
          ...
          $ExpertId = $_SESSION["User_Id"];
          $ViewQuery = "SELECT event.id, fname, lname, profile_pic, email, title, profession, date_published, category, theme, venue, event_descrip, event_date, event_bunner, event_time FROM event INNER JOIN expert_bio ON event.expert_id = expert_bio.id WHERE expert_bio.id = '$ExpertId' ORDER BY date_published DESC LIMIT $ShowPostFrom,5";}
           else{
            $ExpertId = $_SESSION["User_Id"];
            $ViewQuery = "SELECT event.id, fname, lname, profile_pic, email, title, profession, date_published, category, theme, venue, event_descrip, event_date, event_bunner, event_time FROM event INNER JOIN expert_bio ON event.expert_id = expert_bio.id WHERE expert_bio.id = '$ExpertId' ORDER BY date_published DESC LIMIT 0,5";}
          ...
          <div class="event_details">
           ...
           <div class="event-theme">
            ...
            <p>
             ...
             ?>
             <a href="IndividualFullEvent.php?id=<?php echo $EventId ?>">Read More</a>
           </p>
         </div>
         </div>
 ...
 <div class="article-pagination">
      ...
      ?>
      <a href="IndividualEvent.php?Page=<?php echo $Page-1?>"> &laquo; </a> 
      <?php } ?>
      <?php } ?>
    <!-- end Back btn -->
    ...
    <a href="IndividualEvent.php?Page=<?php echo $i; ?>" id="article-pagination-active"><?php echo $i; ?></a>
    <?php
      }else{
    ?>
    <a href="IndividualEvent.php?Page=<?php echo $i; ?>"><?php echo $i; ?></a>
  <?php } ?> <!-- end else -->
<?php } ?>
          <?php } ?> <!-- end forloop -->
    <!-- Forward btn -->
      <?php
        ...
        ?>
           <a href="IndividualEvent.php?Page=<?php echo $Page+1 ?>"> &raquo; </a>
          <?php } ?>
          <?php } ?> <!-- End Forward btn -->
  </div>
      <!-- End Pagination -->
