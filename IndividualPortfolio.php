...
<head>
	...
	<link rel="stylesheet" type="text/css" href="css/IndividualPortfolio.css">
</head>
<body>
   <!-- navigation bar -->
    <!-- heading -->
  ...
  <div class="col-md-3">
     ...
     <?php
        ...
        if ($TotalRaws) {
          ...
          <a href="IndividualService.php?Page=1"><i class="fas fa-box"></i> Service <span style="background-color: #00b4ff; color: #ffffff; padding: 1px 3px; border-radius: 2px;"><?php echo $TotalRaws; ?></span></a>
     <?php } ?> <!-- End if Condition -->
     ...
     <div class="col-md-7">
      <div class="addNewPost-container">
       ...
       <div class="searchForm">
            <form action="IndividualPortfolio.php">
       ...
       //Query when Search Button is Active
           if (isset($_GET["search-btn"])) {
               $Search = $_GET["search"];
               $ExpertId = $_SESSION["User_Id"];
               $ViewQuery = "SELECT portfolio.id, fname, lname, profile_pic, email, title, profession, date_published, portfolio_text, port_bunner, port_title, expert_id FROM portfolio INNER JOIN expert_bio ON portfolio.expert_id = expert_bio.id AND expert_id = '$ExpertId' WHERE date_published LIKE '%$Search%' OR port_title LIKE '%$Search%' OR portfolio_text LIKE '%$Search%'";
          ...
          $ExpertId = $_SESSION["User_Id"];
           $ViewQuery = "SELECT portfolio.id, fname,lname, profile_pic,email,title, profession, date_published, portfolio_text, port_bunner, port_title FROM portfolio INNER JOIN expert_bio ON portfolio.expert_id = expert_bio.id WHERE expert_bio.id = '$ExpertId' ORDER BY date_published DESC LIMIT $ShowPostFrom,5";}
           else{
           $ExpertId = $_SESSION["User_Id"];
           $ViewQuery = "SELECT portfolio.id, fname,lname, profile_pic,email,title, profession, date_published, portfolio_text, port_bunner, port_title FROM portfolio INNER JOIN expert_bio ON portfolio.expert_id = expert_bio.id WHERE expert_bio.id = '$ExpertId' ORDER BY date_published DESC LIMIT 0,5";}
           ...
           <div class="portfolio_content">
            ...
            <p>
             ...
             ?>
             <a href="IndividualFullPortfolio.php?id=<?php echo $PortfolioId ?>">Read More</a>
           </p>            
     ...
     <div class="article-pagination">
      ...
      ?>
      <a href="IndividualPortfolio.php?Page=<?php echo $Page-1?>"> &laquo; </a> 
      <?php } ?>
      <?php } ?>
    <!-- end Back btn -->
    ...
    <a href="IndividualPortfolio.php?Page=<?php echo $i; ?>" id="article-pagination-active"><?php echo $i; ?></a>
    <?php
      }else{
    ?>
    <a href="IndividualPortfolio.php?Page=<?php echo $i; ?>"><?php echo $i; ?></a>
  <?php } ?> <!-- end else -->
<?php } ?>
          <?php } ?> <!-- end forloop -->
    <!-- Forward btn -->
      <?php
        ...
        ?>
           <a href="IndividualPortfolio.php?Page=<?php echo $Page+1 ?>"> &raquo; </a>
          <?php } ?>
          <?php } ?> <!-- End Forward btn -->
      </div>
      <!-- End Pagination -->
