...
<header>
  <div class="row">
    ...
    <section>
      <div class="row">
       ...
       //Query when Search Button is Active
           if (isset($_GET["search-btn"])) {
               ...
               $ViewQuery = "SELECT event.id, fname, lname, profile_pic, email, title, profession, date_published, event_descrip, event_bunner, theme, venue, event_time, event_date, category FROM event INNER JOIN expert_bio ON event.expert_id = expert_bio.id WHERE date_published LIKE '%$Search%' OR fname LIKE '%$Search%' OR event_time LIKE '%$Search%'  OR event_date LIKE '%$Search%' OR theme LIKE '%$Search%' OR venue LIKE '%$Search%' OR category LIKE '%$Search%'";
          ...
          <div class="row">
            ...
            <div class="col-md-5">
             ...
             <h1><?php echo nl2br($Theme); ?></h1>
              ...
              <p> <?php
                 if (strlen($EventDescription)>150) {
                    $EventDescription = substr($EventDescription, 0,150);
                 }
               echo nl2br($EventDescription);
                ?>   
              </p>
               <a href="fullevent.php?id=<?php echo $EventId; ?>">More Details</a>
            </div>
          </div>
      ...
      ?>
      <a href="events.php?Page=<?php echo $Page-1?>"> &laquo; </a> 
      <?php } ?>
      <?php } ?>
    <!-- end Back btn -->
        ...
        <?php
         $QueryPagination = "SELECT COUNT(*) FROM event";
         $ExcutePagination = mysqli_query($dbcon, $QueryPagination);
         $RowPagination = mysqli_fetch_array($ExcutePagination);
         $TotalEvent=array_shift($RowPagination);
        // echo $TotalPosts;
         $EventPagination=$TotalEvent/5;
         $EventPagination=ceil($EventPagination);
        // echo $PostPerPage;
         for ($i=1;$i<=$EventPagination;$i++){
          if (isset($Page)) {
    ...
    <a href="events.php?Page=<?php echo $i; ?>" id="article-pagination-active"><?php echo $i; ?></a>
    <?php
      }else{
    ?>
 ...
 <!-- Forward btn -->
      <?php
         if (isset($Page)) {
           ...
           if ($Page+1<=$EventPagination) {

        ?>
           <a href="events.php?Page=<?php echo $Page+1?>"> &raquo; </a>
          <?php } ?>
          <?php } ?> <!-- End Forward btn -->
