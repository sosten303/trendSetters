...
if(isset($_POST["Submit"])){
  ...
  $ClientName = mysqli_real_escape_string($dbcon,trim($_POST["client_name"]));
  $AboutClient = mysqli_real_escape_string($dbcon,trim($_POST["about_client"]));
  $ClientBunner = $_FILES["client_bunner"]["name"];
  $Target="ClientPic/".basename($_FILES["client_bunner"]["name"]);
 ...
 // $DateTime = strftime("%Y-%m-%d %H:%M:%S",$CurrentTime);
  ...
  if (isset($_GET["EditClient"])) {
     $EditClientFromUrl = $_GET["EditClient"];
  // global $dbcon;
  // $ExpertId = $_SESSION["User_Id"];
  $Query= "UPDATE client SET client_name = '$ClientName', client_bunner = '$ClientBunner', about_client = '$AboutClient'
  WHERE service.id = '$EditClientFromUrl'";
  $Excute=mysqli_query($dbcon,$Query);
  move_uploaded_file($_FILES["client_bunner"]["tmp_name"], $Target);
 if ($Excute) {
     $_SESSION["SuccessMessage"]="Client Updated successfully";
     Redirect_to("IndividualClient.php?Page=1");
  }else{
    $_SESSION["ErrorMessage"]=" Failed to Update Client Try Again !";
    Redirect_to("EditClient.php");
  }
}
     ...
     <div class="col-md-8">
      <div class="addNewPost-container">
       ...
       <?php 
          ...
          $EditClientFromUrl = $_GET["EditClient"];
          $Query = "SELECT * FROM client INNER JOIN expert_bio ON client.expert_id = expert_bio.id WHERE client.id = '$EditClientFromUrl'";
        ...
        <form action="EditClient.php?EditClient=<?php echo $EditClientFromUrl; ?>" method="post" enctype="multipart/form-data">
           <p></p>
         <span>Existing Client Picture / Bunner :</span> 
         <img src="ClientPic/<?php echo $ClientBunner; ?>" with=270px height=100px; />
         <p></p>
         <label>Change Client Picture / Bunner :</label>
         <i class="fas fa-camera"></i> <input type="File" name="client_bunner" />
         <p></p>
         <label for="Title">Client Name :</label>
         <input type="text" name="client_name" value="<?php echo $ClientName ?>" />
         <p></p>
          <label>About Service :</label>
         <p></p>
         <textarea name="about_client" placeholder="Edit Client details"><?php echo $AboutClient; ?></textarea>
         <p></p>
