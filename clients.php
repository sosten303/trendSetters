...
<div class="col-md-4">
     ...
     <div class="client-wrapper">
         ...
         <div class="col-md-2"></div>
           <div class="col-md-8">
              ...
              <?php 
               ...
               // intialise pagenation
              //Query when Search Button is Active
           if (isset($_GET["search-btn"])) {
               $Search=$_GET["search"];
               $ExpertId = $_SESSION["User_Id"];
                $ViewQuery = "SELECT client.id, fname, lname, profile_pic, email, title, profession, date_published, client_name, client_bunner, about_client, expert_id FROM client INNER JOIN expert_bio ON client.expert_id = expert_bio.id WHERE date_published LIKE '%$Search%' OR client_name LIKE '%$Search%' OR about_client LIKE '%$Search%'";
           }
       // Query when pagination is Active i.e expert-articles.php?Page=1
        ...
        // $ShowPostFrom=($Page*5)-5; if search page=2, multply it by 5 then subtract the result by 5=5 articles per page
          $ViewQuery = "SELECT client.id, fname, lname, profile_pic, email, title, profession, date_published, client_name, client_bunner, about_client FROM client INNER JOIN expert_bio ON client.expert_id = expert_bio.id ORDER BY date_published DESC LIMIT $ShowPostFrom,5";}
           else{
       
          $ViewQuery = "SELECT client.id, fname, lname, profile_pic, email, title, profession, date_published, client_name, client_bunner, about_client FROM client INNER JOIN expert_bio ON client.expert_id = expert_bio.id ORDER BY date_published DESC LIMIT 0,5";}
           ...
           ?>
               <div class="client-container">
                ...
                <div class="client-details">
                 <h1><?php echo htmlentities($ClientName);  ?></h1>
                 <p><b>Expert Connected :</b> <?php echo htmlentities($Title); ?>
                 <?php echo htmlentities($Fname); ?>
                 <?php echo htmlentities($Lname); ?>
               </p>
                 <p><b>About Client :</b> <?php
                  //limit the amount of text of $AboutClient 
                 if (strlen($AboutClient)>150) {
                  ...
                  echo nl2br($AboutClient);
                   ...
                   ?></p>
                 <a href="fullclient.php?id=<?php echo $ClientId;  ?>">More Details</a>
                 <p id="client-border"></p>
               </div>
            </div>
                 <?php } ?> <!-- End While Loop -->
 ...
 <div class="client-pagination">
      ...
      <!-- Back btn -->
        ...
        ?>
          <a href="clients.php?Page=<?php echo $Page-1?>"> &laquo; </a> 
            <?php } ?>
            <?php } ?>
    <!-- end Back btn -->
        ...
        <?php
         $QueryPagination = "SELECT COUNT(*) FROM client";
         $ExcutePagination = mysqli_query($dbcon, $QueryPagination);
         $RowPagination = mysqli_fetch_array($ExcutePagination);
         $TotalClients = array_shift($RowPagination);
        // echo $TotalPosts;
         $ClientsPagination = $TotalClients/5;
         $ClientsPagination = ceil($ClientsPagination);
        // echo $PostPerPage;
         for ($i = 1; $i <= $ClientsPagination; $i++){
          if (isset($Page)) {
           if ($i==$Page) {
    ...
    <a href="clients.php?Page=<?php echo $i; ?>" id="client-pagination-active"><?php echo $i; ?></a>
    <?php
      }else{
    ?>
    <a href="clients.php?Page=<?php echo $i; ?>"><?php echo $i; ?></a>
  <?php } ?> <!-- end else -->
  <?php } ?>
          <?php } ?> <!-- end forloop -->
 ...
 <!-- Forward btn -->
      <?php
         if (isset($Page)) {
           ...
           if ($Page+1<=$ClientsPagination) {

        ?>
           <a href="clients.php?Page=<?php echo $Page+1?>"> &raquo; </a>
          <?php } ?>
          <?php } ?> <!-- End Forward btn -->
