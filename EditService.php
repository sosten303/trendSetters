...
if(isset($_POST["Submit"])){
  ...
  $ServiceTitle = mysqli_real_escape_string($dbcon,trim($_POST["service_title"]));
  $Service = mysqli_real_escape_string($dbcon,trim($_POST["service_text"]));
  $ServiceBunner = $_FILES["service_bunner"]["name"];
  $Target="ServicePic/".basename($_FILES["service_bunner"]["name"]);
 ...
 // $DateTime = strftime("%Y-%m-%d %H:%M:%S",$CurrentTime);
  ...
  if (isset($_GET["EditService"])) {
     $EditServiceFromUrl = $_GET["EditService"];
  // global $dbcon;
  // $ExpertId = $_SESSION["User_Id"];
  $Query= "UPDATE service SET service_title = '$ServiceTitle', service_bunner = '$ServiceBunner', service_text = '$Service'
  WHERE service.id = '$EditServiceFromUrl'";
  $Excute=mysqli_query($dbcon,$Query);
  move_uploaded_file($_FILES["service_bunner"]["tmp_name"], $Target);
 if ($Excute) {
     $_SESSION["SuccessMessage"]="Service Updated successfully";
     Redirect_to("IndividualService.php?Page=1");
  }else{
    $_SESSION["ErrorMessage"]=" Failed to Update Service Try Again !";
     ...
     <div class="col-md-8">
      <div class="addNewPost-container">
       ...
       <?php 
          ...
          $EditServiceFromUrl = $_GET["EditService"];
          $Query = "SELECT * FROM service INNER JOIN expert_bio ON service.expert_id = expert_bio.id WHERE service.id = '$EditServiceFromUrl'";
          $Execute = mysqli_query($dbcon,$Query);
           while ($DataRows = mysqli_fetch_array($Execute)) {
             $ServicetId = $DataRows["id"];
        ...
        <form action="EditService.php?EditService=<?php echo $EditServiceFromUrl; ?>" method="post" enctype="multipart/form-data">
           <p></p>
         <span>Existing Service Picture:</span> 
         <img src="ServicePic/<?php echo $ServiceBunner; ?>" with=270px height=100px; />
         <p></p>
         <label>Change Service Picture :</label>
         <i class="fas fa-camera"></i> <input type="File" name="service_bunner" />
         <p></p>
         <label for="Title">Title :</label>
         <input type="text" name="service_title" value="<?php echo $ServiceTitle ?>" />
         <p></p>
          <label>About Service :</label>
         <p></p>
         <textarea name="service_text" placeholder="Edit Service"><?php echo $Service; ?></textarea>
         <p></p>
