...
if(isset($_POST["Submit"])){
 ...
 // $DateTime = strftime("%Y-%m-%d %H:%M:%S",$CurrentTime);
  ...
  if (isset($_GET["DeleteService"])) {
     $DeleteServiceFromUrl = $_GET["DeleteService"];
  // global $dbcon;
  // $ExpertId = $_SESSION["User_Id"];
  $Query= "DELETE FROM service WHERE service.id = '$DeleteServiceFromUrl'";
  $Excute=mysqli_query($dbcon,$Query);
  move_uploaded_file($_FILES["service_bunner"]["tmp_name"], $Target);
 if ($Excute) {
     $_SESSION["SuccessMessage"]="Service Deleted successfully";
     Redirect_to("IndividualService.php?Page=1");
  }else{
    $_SESSION["ErrorMessage"]=" Failed to Delete Service Try Again !";
    Redirect_to("DeleteService.php");
  }
}
     ...
     <div class="col-md-8">
      <div class="addNewPost-container">
       ...
       <?php 
          ...
          $DeleteServiceFromUrl = $_GET["DeleteService"];
          $Query = "SELECT * FROM service INNER JOIN expert_bio ON service.expert_id = expert_bio.id WHERE service.id = '$DeleteServiceFromUrl'";
        ...
        <form action="DeleteService.php?DeleteService=<?php echo $DeleteServiceFromUrl; ?>" method="post" enctype="multipart/form-data">
           <p></p>
         <span>Service Picture:</span> 
         <img disabled src="ServicePic/<?php echo $serviceBunner; ?>" with=270px height=100px; />
         <p></p>
         <label for="Service">Service Picture :</label>
         <i class="fas fa-camera"></i> <input disabled type="File" name="service_bunner" id="Service" />
         <p></p>
         <label for="Title">Title :</label>
         <input disabled type="text" name="service_title" value="<?php echo $ServiceTitle ?>" id="Title"/>
         <p></p>
          <label>Service :</label>
         <p></p>
         <textarea disabled name="service_text"><?php echo $Service; ?></textarea>
         <p></p>
