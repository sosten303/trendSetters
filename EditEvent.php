...
if(isset($_POST["Submit"])){
   ...
   $Theme = mysqli_real_escape_string($dbcon,trim($_POST["theme"]));
  $Venue = mysqli_real_escape_string($dbcon,trim($_POST["venue"]));
  $Category = mysqli_real_escape_string($dbcon,trim($_POST["category"]));
  $EventDate = mysqli_real_escape_string($dbcon,trim($_POST["event_date"]));
  $UpcomingDate = mysqli_real_escape_string($dbcon,trim($_POST["event_time"]));
  $EventDesription = mysqli_real_escape_string($dbcon,trim($_POST["event_descrip"]));
  $Target = "EventPic/".basename($_FILES["event_bunner"]["name"]);
  $EventBunner = $_FILES["event_bunner"]["name"];
  $ExpertId = $_SESSION["User_Id"];

  if (isset($_GET["EditEvent"])) {
     $EditEventFromUrl = $_GET["EditEvent"];
  // global $dbcon;
  // $ExpertId = $_SESSION["User_Id"];
  $Query= "UPDATE event SET theme = '$Theme', venue = '$Venue', category = '$Category', event_descrip = '$EventDesription', event_bunner = '$EventBunner', event_date = '$EventDate', event_time = '$UpcomingDate'
  WHERE event.id = '$EditEventFromUrl'";
  $Excute=mysqli_query($dbcon,$Query);
  move_uploaded_file($_FILES["event_bunner"]["tmp_name"], $Target);
 if ($Excute) {
     $_SESSION["SuccessMessage"]="Client Updated successfully";
     Redirect_to("IndividualEvent.php?Page=1");
  }else{
    $_SESSION["ErrorMessage"]=" Failed to Update Client Try Again !";
    Redirect_to("EditEvent.php");
  }
}
     ...
     <a href="updateProfile.php"><i class="fas fa-user"></i>Update Profile</a>
      <?php
        ...
        if ($TotalRaws) {
          ...
          <a href="IndividualArticles.php?Page=1"  id="dashboard-active"><i class="fas fa-newspaper"></i> Articles <span style="background-color: #00b4ff; color: #ffffff; padding: 1px 3px; border-radius: 2px;"><?php echo $TotalRaws; ?></span></a>
     ...
     <div class="col-md-8">
      <div class="addNewPost-container">
       ...
       <?php 
          ...
          $EditEventFromUrl = $_GET["EditEvent"];
          $Query = "SELECT * FROM event INNER JOIN expert_bio ON event.expert_id = expert_bio.id WHERE event.id = '$EditEventFromUrl'";
        ...
        <div>
          <form action="EditEvent.php?EditEvent=<?php echo $EditEventFromUrl; ?>" method="post" enctype="multipart/form-data"/>
             <span>Existing Event Picture / Bunner :</span> 
            <img src="EventPic/<?php echo $EventBunner; ?>" with=270px height=100px; />
            <label for="EventImage">Event Image</label>
            <p></p>
            <i class="fas fa-camera"></i> <input type="File" name="event_bunner" id="EventImage"/>
            <p></p>
            <label for="Category">Category</label>
            <p></p>
            <input type="text" name="category" id="Category" placeholder="Change Category" value="<?php echo $Category ?>" />
            <p></p>
            <label for="Theme">Theme</label>
            <p></p>
            <input type="text" name="theme" id="Theme" placeholder="Change Theme" value="<?php echo $Theme ?>" />
            <p></p>
            <label for="Venue">Venue</label>
            <p></p>
            <i class="fas fa-map-marker-alt"></i> <input type="text" name="venue" id="Venue" placeholder="Change Venue" value="<?php echo $Venue ?>" />
            <p></p>
            <label for="Time">Time</label>
            <p></p>
            <i class="fas fa-clock"></i> <input type="text" name="event_time" id="Time" placeholder=" from 00:00 to 00:00 AM or PM" value="<?php echo $EventDate ?>" />
            <p></p>
             <label for="DateUpcoming">Up Coming Event</label>
             <p></p>
            <i class="fas fa-calendar-alt"></i> <input type="date" name="event_date" id="DateUpcoming" placeholder=" dd/mm/yy" value="<?php echo $EventDate ?>" />
            <p></p>
             <label for="Description"><i class="fas fa-pen"></i> Description</label>
             <p></p>
            <textarea type="text" name="event_descrip" id="Description" placeholder=" Any Description, Intro or special Remarks" value="<?php echo $EventDescrip ?>" ></textarea>
            <p></p>
